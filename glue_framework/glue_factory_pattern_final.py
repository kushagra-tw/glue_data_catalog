...
def main():
    ...
    database_name = 'jcp-edp-silver'
    database_manager.create_database(database_name)


    with open('./customer_identity.json') as file:
    ...
    #
    crawler_manager.create_crawler_using_glue_table('silver_layer_crawler',database_name,table_name)



    # print('running crawler')
    crawler_manager.run_crawler('silver_layer_crawler')
if __name__ == "__main__":
    main()


#once data is updated in s3 bucket then we enable the trigger the enable the trigger (event based trigger)

#event based trigger on the s3? Once the data is in the s3 it will trigger the crawler...

#approach for table creation and database creation....
#can be achieved using CI pipeline.


#seperate dag for this only one task which will have this task only..

#creation part
#database :- simple sql script
#table :- simple sql script

#crawler :- python
    #creation
    #run of crawler :- task inside the main dag of the main pipeline.
...
#add new partitions :-
#run the crawler
#run create or replace script again
#query the new partition we added.

#change the underlying partition
#for each table create a crawler
#add config outside.

#try alter statement with parquet also.
#pyspark job should be able to tell this and see
#or
# how to find the latest partition?

#hudie try with alter command.